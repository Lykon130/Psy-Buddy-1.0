...
| L14 Hybrid Memory (crosscutting) | Relational Memory | Memory Engine | **Minimal** — one auto-updated text blob per user, no memory classes, no confidence/provenance, not visible or editable by the user, no vector/semantic store despite FAISS being in the architecture diagrams |
...
| L17 Crisis Safety Core (Safety/Governance) | Safety Layer | Safety AI / Crisis Detection | **Not started in the pipeline** — no risk detection, no escalation, no resource surfacing during conversation. One static onboarding disclaimer exists (`disclaimer_screen.dart`: "not a substitute for professional medical advice," shown once at signup) — real, but it's a legal notice, not a safety system |
